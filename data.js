// ---- seed data (used only the very first time, before anyone adds their own) ----
const SEED_DISHES=[
...
const STORE_KEY="dealbook.customDishes";
...
// dishes people add locally are kept separate from the seed set, then merged
let CUSTOM_DISHES = loadCustom();
let DISHES = [...SEED_DISHES, ...CUSTOM_DISHES];

function addDish(d){
  CUSTOM_DISHES.push(d);
  saveCustom(CUSTOM_DISHES);
  DISHES = [...SEED_DISHES, ...CUSTOM_DISHES];
}
